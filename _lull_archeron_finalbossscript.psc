...
;updated 8/13/21 vitrial. Make clearable, replace getplayer with playerref
;For hitting
...
;Post Scene
location property lullCaveLocation auto
...
Event OnHit(ObjectReference akAggressor, Form akSource, Projectile akProjectile, bool abPowerAttack, bool abSneakAttack,   bool abBashAttack, bool abHitBlocked)
    ...
    if(!archeronWeak) ;Archeron does NOT have purple glow/vulnerability to shield
        ...
        if(akSpell == lullianProjectile)
            UnregisterForUpdate()
            explosionFX.Cast(self, playerRef)
            archeronHits += 1
...
Function RandomTeleport()
        ...
        if(randomLocation == 0)
            ...
            RandomEffect()
            archeron.StartCombat(playerRef)
...
Function TransToRamon()
    ...
    Utility.Wait(0.5)
    MQ07.setstage(8)
    lullCaveLocation.setCleared(true)
EndFunction
